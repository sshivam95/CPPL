...
class UCB:
    ...
    def step(self):
        if self.time_step == 0:
            ...
            try:
                V_hat = np.asarray((1 / self.time_step) * self.grad_op_sum).astype("float64")
                S_hat = np.asarray((1 / self.time_step) * self.hess_sum).astype("float64")
                ...
                for i in range(self.n_arms):
                    M_i = np.exp(2 * np.dot(self.theta_bar, self.context_matrix[i, :])) * np.dot(self.context_matrix[i, :], self.context_matrix[i, :])  # M_t ^(i) (theta)
                    self.confidence_t[i] = self.omega * np.sqrt(
                        (2 * np.log(self.time_step) + self.degree_of_freedom + 2 * np.sqrt(self.degree_of_freedom * np.log(self.time_step)))
