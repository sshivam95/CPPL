...
def start(params,timelimit,filename,solver):
    ...
    if solver == 'cadical':
        ...
        proc = subprocess.Popen(['solvers/./cadical', '--no-witness', 
                                 *params, '-t', str(timelimit), 
                                 f'{filename}'], stdout=PIPE)
        return proc

    elif solver == 'glucose':

        proc = subprocess.Popen(['solvers/./glucose_static', 
                                 '-cpu-lim='+str(timelimit), *params, 
                                 f'{filename}'],stdout=PIPE)
        return proc
        ...
        proc = subprocess.Popen(['python3', 'solvers/cplex_solve.py',
                                 '-d', directory, 
                                 '-f', f'{filename}', 
                                 '-to', str(timelimit),
                                 '-g', str(params)], stdout=PIPE)
        ...
        if line != b'':
            ...
            if b[2] == 'c':
                ...
                if len(b) > 11 and len(b[11]) > 5:
                    interim = [int(b[2][1:]),int(b[3][:-2]),int(b[4][1:]),
                               int(b[5][1:]),int(b[6][1:-2]),int(b[7]),
                               int(b[8]),int(b[9]),int(b[10][:-1]),
                               float(b[11][:-4])]
                    return interim
        else:
            return 'No output'
