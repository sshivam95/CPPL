...
def get_contenders(directory, filename, pca_obj_inst, pca_obj_params, jfm, theta_bar, 
                   t, k, Y_t, S_t, grad, hess_sum, grad_op_sum, omega, solver, Pool,
                   tracking_Pool, min_max_scaler, standard_scaler, pl, param_value_dict,
                   json_param_file,exp):
    ...
    # construct X_t (context specific (instance information) feature matrix ( and parameterizations information))
    n = params.shape[0]
    d = len(theta_bar)
    X_t = np.zeros((n, d))
    for i in range(n): 
    ...
    if t >= 1:
        ...
        try:
            ...
            for i in range(n):
                M_i = np.exp(2*np.dot(theta_bar,X_t[i,:]))*np.dot(X_t[i,:],X_t[i,:])

                c_t[i] = omega*np.sqrt((2*np.log(t)+d+2*np.sqrt(d*np.log(t)))*np.linalg.norm(Sigma_hat_sqrt*M_i*Sigma_hat_sqrt, ord=2))
    ...
    if t >= 1:
        for p1 in range(n):
            for p2 in range(n):
                if p2 != p1 and v_hat[p2] - c_t[p2] >= v_hat[p1] + c_t[p1] and (not (p1 in discard)):
                    discard.append(p1)
                    break
        ...
        if len(discard) > 0:
            ...
            parametrizations = log_on_huge_params.log_space_convert(solver,pl,parametrizations,json_param_file)

            best_candid = log_on_huge_params.log_space_convert(solver,pl,random_genes.One_Hot_decode(parametrizations[S_t[0],],solver,
                                                       param_value_dict=param_value_dict,json_param_file=json_param_file),
                                                       json_param_file,exp=True)

            second_candid = log_on_huge_params.log_space_convert(solver,pl,random_genes.One_Hot_decode(parametrizations[S_t[1],],solver,
                                                        param_value_dict=param_value_dict,json_param_file=json_param_file),
                                                        json_param_file,exp=True)

            new_candids_transformed, new_candids = parallel_evolution_and_fitness(k,new_candids_size,S_t,params,
                                                                                  solver,param_value_dict,json_param_file,
                                                                                  random_genes.genes_set,
                                                                                  random_genes.One_Hot_decode,
                                                                                  log_on_huge_params.log_space_convert,
                                                                                  Pool,pl,best_candid,second_candid)
            ...
            for i in range(len(best_new_candids_ind)):
                genes = random_genes.getParamsStringFromNumericParams(log_on_huge_params.log_space_convert(
                                                                      solver,pl,random_genes.One_Hot_decode(
                                                                      new_candids[best_new_candids_ind[i]],
                                                                      solver,param_value_dict=param_value_dict,
                ...
                setParam.setParams('contender_'+str(discard[i]),genes,solver,json_param_file)
                Pool['contender_'+str(discard[i])] = genes
                tracking_Pool.info(Pool)

            contender_list = contender_list_including_generated(Pool,solver,pl,params,json_param_file,
                                                                theta_bar,jfm,min_max_scaler,pca_obj_params,
                                                                standard_scaler,pca_obj_inst,directory,filename,k)
    ...
    if t == 0:
        ...
        setParam.setParams('contender_0',genes,solver,json_param_file)
...
def contender_list_including_generated(Pool,solver,pl,params,json_param_file,theta_bar,jfm,min_max_scaler,
                                       pca_obj_params,standard_scaler,pca_obj_inst,directory,filename,k):
...
def parallel_evolution_and_fitness(k,new_candids_size,S_t,params,solver,param_value_dict,
                                   json_param_file,genes_set,One_Hot_decode,log_space_convert,
                                   Pool,pl,best_candid,second_candid):
    
    new_candids = np.zeros(shape=(new_candids_size,len(random_genes.One_Hot_decode(params[S_t[0],],solver,
                                                                                   param_value_dict=param_value_dict,
                                                                                   json_param_file=json_param_file))))
    params_length = len(random_genes.One_Hot_decode(params[S_t[0],],solver,
                                                    param_value_dict=param_value_dict,
                                                    json_param_file=json_param_file))
    ...
    for i in range(len(all_steps)):
        step += all_steps[i]
        pool.apply_async(evolution_and_fitness,(best_candid,second_candid,
                                                len(new_candids[0]),all_steps[i],
                                                params_length,genes_set,
                                                One_Hot_decode,log_space_convert,
                                                solver,json_param_file,Pool,
                                                param_value_dict,pl), callback=saveResult)
...
def evolution_and_fitness(best_candid,second_candid,new_candids,new_candids_size,
                          params_length,genes_set,One_Hot_decode,log_space_convert,
                          solver,json_param_file,Pool,param_value_dict,pl):
    ...
    for counter in range(new_candids_size):        
        ...
        mutation_genes = random_genes.One_Hot_decode(genes,solver,
                                                     param_value_dict=param_value_dict,
                                                     json_param_file=json_param_file)   
    ...
    new_candids = random_genes.One_Hot_decode(new_candids,solver,
                                              json_param_file=json_param_file,
                                              reverse=True)

    new_candids_transformed = log_on_huge_params.log_space_convert(solver,pl,
                                                                   new_candids,
                                                                   json_param_file)
...
def read_parametrizations(pool,solver,contender=None,json_param_file=None):
    ...
    if contender != None:
        ...
        new_params, param_value_dict = param_read_from_dict(solver,contender,
                                                            params,paramNames)
        ...
        for key in newPool:
            ...
            new_params, param_value_dict = param_read_from_dict(solver,newPool[key],
                                                                params,paramNames)
