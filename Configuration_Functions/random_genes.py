        ...
        if params[paramNames[i]]["paramtype"] == "categorical":
            ...
            if "distribution" in params[paramNames[i]]:
                if params[paramNames[i]]["distribution"] == "log":
                            ...
                            if params[paramNames[i]]["includezero"] == True:
                                ...
                                if "probabilityzero" in params[paramNames[i]]:
                                    if params[paramNames[i]]["probabilityzero"] == True:
                                        probabzero = params[paramNames[i]][
                                            "probabilityzero"
                                        ]
                                    else:
                    ...
                    if "logonpos" in params[paramNames[i]]:
                        ...
                        if "probabpos" in params[paramNames[i]]:
                            ...
                            if probabzero and "probabneg" in params[paramNames[i]]:
                                probabpos = (
                                    1 - probabzero - params[paramNames[i]]["probabneg"]
                                )
                            elif probabzero:
                    ...
                    if logonpos:
                        high = math.exp(
                            np.random.uniform(0.000000001, logmaxVal, size=1)
                        )
                    else:
                    ...
                    if splitbydefault:
                        if default == 0:
                            high = math.exp(
                                np.random.uniform(default, logmaxVal, size=1)
                            )
                        else:
                            high = math.exp(
                                np.random.uniform(math.log(default), logmaxVal, size=1)
                            )
                    if logonneg:
                        low = (
                            -math.exp(
                                np.random.uniform(0.000000001, -logminVal, size=1)
                            )
                            - minVal
                        )
                    else:
                        ...
                        genes[i] = float(
                            choice(
                                [
                                    math.exp(
                                        np.random.uniform(
                                            math.log(minVal), logmaxVal, size=1
                                        )
                                    )
                                ],
                                1,
...
def One_Hot_decode(
    ...
    if (
        not reverse
    ):  # One-Hot decoding here (one-hot paramters back to solver specific representation)
                ...
                if params[paramNames[i]]["paramtype"] == "categorical":
                    if params[paramNames[i]]["valtype"] == "int":
                        ...
                        if (maxVal - minVal + 1) > 2:
                            ...
                            real_vector.append(
                                [minVal + t for t in range(maxVal - minVal + 1)]
                            )
