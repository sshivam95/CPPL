        ...
        for i in range(len(paramset)):
            for j in range(len(maxval_indices)):
                if float(paramset[i][maxval_indices[j]]) > 0:
                    paramset[i][maxval_indices[j]] = math.log(
                        float(paramset[i][maxval_indices[j]])
                    )
            ...
            for j in range(len(minval_indices)):
                if float(paramset[i][minval_indices[j]]) < 0:
                    paramset[i][minval_indices[j]] = -math.log(
                        float(-paramset[i][minval_indices[j]])
                    )
    ...
    if exp:
        ...
        for j in range(len(maxval_indices)):
            if float(paramset[maxval_indices[j]]) > 0:
                paramset[maxval_indices[j]] = math.exp(
                    float(paramset[maxval_indices[j]])
                )
        ...
        for j in range(len(minval_indices)):
            if float(paramset[minval_indices[j]]) < 0:
                paramset[minval_indices[j]] = -math.exp(
                    float(-paramset[minval_indices[j]])
                )
